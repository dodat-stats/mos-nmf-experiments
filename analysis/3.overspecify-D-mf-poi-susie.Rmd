---
title: "3. MF Poisson SuSiE With Overspecified D"
...
The previous page fits MF-Poisson-SuSiE with the correct value \(D = 3\). This
page stress-tests the same model with \(D = 5\), while the data-generating
mechanism still has at most three active factors per observation.

The question is whether the two extra single-effect dimensions become harmless.
A benign extra dimension should either have small posterior expected loading
mass, diffuse posterior assignment probabilities \(\bar\gamma_{idk}\), or both.
This makes overfitted \(D\) a data-wise diagnostic: for each observation \(i\),
we inspect the fitted dimensions \(d=1,\ldots,D\) and ask which dimensions carry
real loading mass.
