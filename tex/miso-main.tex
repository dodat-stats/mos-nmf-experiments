...
Distinctness is important. A threshold-and-recycle rule would first retain an
ordered set
\[
  A_s(\tau)
  =
  (a_{s1},\ldots,a_{sq_s})
  =
  \{r_s(j):c_{s,r_s(j)}\geq\tau\}
\]
and then fill the $D$ dimensions according to
\[
  \kappa_{sd}^{\mathrm{recycle}}
  =
  a_{s,\,1+((d-1)\bmod q_s)}.
\]
When only one factor passes the threshold, this assigns the same factor to all
$D$ dimensions. Conditional on motif $s$, the resulting loading is
\[
  L_i
  =
...
\]
which is a one-dimensional ray regardless of the nominal value of $D$. MiSo
therefore initializes the top $D$ directions without replacement. If $D$ is
over-specified, unnecessary directions are removed later through the
empirical-Bayes dimension diagnostic rather than duplicated at initialization.
...
\begin{enumerate}
  ...
  \item ablations of MiSo with fixed $F$, learned $F$, and different
...
default therefore initializes the top $D$ center factors without replacement;
recycling a shorter thresholded list can duplicate active dimensions and create
an artificial rank-one motif. A full paper needs
...
stability across initializations, and external agreement with cell annotations
...
\begin{enumerate}
  ...
  \item distinct top-$D$ initialization versus threshold-and-recycle and
