## LEGACY HARD-MOTIF EXPERIMENT PROTOTYPE. Not part of the frozen MiSo API.
## Use miso() from miso.R instead.

## Mixture-of-submanifolds Poisson factor model.
##
## This is an initial, deliberately concrete version of the MiSo layer.  It
...
simulate_miso_data <- function(N = 240, M = 300, K = 6,
...
update_miso_priors <- function(components, omega, alpha0, lambda0,
                              min_shape = 1e-3, max_shape = 1e4,
                              eps = 1e-12) {
  S = length(components)
  D = ncol(alpha0)
...
update_miso_F <- function(Y, F, motifs, components, omega,
...
update_miso_motifs <- function(Y, F, motifs, components, omega, alpha0,
...
miso_poi_susie_fixed_motifs <- function(Y, F, motifs, max_iters = 50,
  ...
  for (iter in seq_len(max_iters)) {
    ...
    if (update_prior) {
      prior_fit = update_miso_priors(component_fit$components, omega,
...
miso_poi_susie <- function(Y, K, S, D, max_iters = 50, n_inner = 8,
  ...
  miso_fit = miso_poi_susie_fixed_motifs(
    Y = Y,
    F = mf_fit$F,
  ...
  miso_fit$mf_fit = mf_fit
  miso_fit$motif_init = motif_init
  miso_fit
}
