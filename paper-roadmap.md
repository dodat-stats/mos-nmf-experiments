...
loadings, not only about improving NMF reconstruction. The central claim
should be:

> MoS-NMF learns repeated sparse loading motifs, clusters samples by
...
-   MoS-NMF pools that structure across samples through motif-level
    latent submanifolds.
-   MoS-NMF represents motif dimensions with posterior motif
...
| MoS-NMF | `tex/mos-nmf.tex`, `code/mos-nmf.R`, `analysis/mos-nmf-alternating.Rmd` | Target model with global motif posterior `gamma_bar[s, d, k]`. |
...
-   MoS-NMF improves the block scenario in the single-seed
...
-   The three-seed pilot is more mixed in the block scenario. MoS-NMF
...
-   The no-anchor tree scenario is hard for MF-Poisson-SuSiE factor
    recovery.
-   In the compact benchmark, MoS-NMF improves support accuracy in
...
-   In the three-seed pilot, the support-recovery advantage is more
    stable: MoS-NMF averages about `0.76` support accuracy, while
...
-   This is currently the strongest positive recovery setting for
    MoS-NMF. It suggests motif sharing helps most when sparsity patterns
...
-   MoS-NMF can express uncertainty across duplicate candidate
...
-   MoS-NMF with repeated surplus motif dimensions does not
...
-   The MoS-NMF code now exposes an ARD-style diagnostic through
    the empirical-Bayes prior mean `alpha^0_{sd} / beta^0_{sd}` in the
    manuscript notation; in code this is stored as `alpha0[s,d] / lambda0[s,d]`. In the
...
3.  Add a real-data example where sparsity motifs have substantive
    meaning.
